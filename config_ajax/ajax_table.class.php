...
require_once('../../configs/config_const.php');
...
class ajax_table {
  ...
  private function dbconnect() {
    ...
    mysql_select_db(MYSQL_EMAILDB,$conn)
  ...
  function getRecords(){
	$this->res = mysql_query("select * from match_config");
  ...
  function save($data){
	if(count($data)){
		...
		mysql_query("insert into match_config (".implode(",",array_keys($data)).") values ('".$values."')");
  ...
  function delete_record($id){
	 if($id){
		mysql_query("delete from match_config where id = $id limit 1");
  ...
  function update_record($data){
	if(count($data)){
		...
		$str = substr($str,0,-1);
		$sql = "update match_config set $str where id = $id limit 1";
  ...
  function update_column($data){
	if(count($data)){
		$id = $data['rid'];
		unset($data['rid']);
		$sql = "update match_config set ".key($data)."='".$data[key($data)]."' where id = $id limit 1";
		$res = mysql_query($sql);
		if(mysql_affected_rows()) return $id;
		return 0;
