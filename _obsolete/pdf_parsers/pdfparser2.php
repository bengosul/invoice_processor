...
function getCharTransformations(&$transformations, $stream) {
 preg_match_all("#([09]+)\s+beginbfchar(.*)endbfchar#ismU", $stream, $chars, PREG_SET_ORDER);
 preg_match_all("#([09]+)\s+beginbfrange(.*)endbfrange#ismU", $stream, $ranges, PREG_SET_ORDER);

for ($j = 0; $j < count($chars); $j++) {
 $count = $chars[$j][1];
 $current = explode("\n", trim($chars[$j][2]));
 for ($k = 0; $k < $count && $k < count($current); $k++) {
 if (preg_match("#<([09a-f]{2,4})>\s+<([09a-f]{4,512})>#is", trim($current[$k]), $map))
 ...
 $current = explode("\n", trim($ranges[$j][2]));
 for ($k = 0; $k < $count && $k < count($current); $k++) {
 if (preg_match("#<([09a-f]{4})>\s+<([09a-f]{4})>\s+<([09a-f]{4})>#is", trim($current[$k]), $map)) {
 $from = hexdec($map[1]);
 $to = hexdec($map[2]);
 $_from = hexdec($map[3]);
...
for ($m = $from, $n = 0; $m <= $to; $m++, $n++)
 ...
 } elseif (preg_match("#<([09a-f]{4})>\s+<([09a-f]{4})>\s+\[(.*)\]#ismU", trim($current[$k]), $map)) {
 $from = hexdec($map[1]);
 $to = hexdec($map[2]);
